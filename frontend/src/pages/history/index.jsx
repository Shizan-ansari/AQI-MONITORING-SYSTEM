...
const pollutantChart = {
  ...
  datasets: [
    {
      label: "PM2.5",
      data: history.map(item => item.pollutants?.pm2_5 ?? 0),
      backgroundColor: "#ef4444"
    },
    {
      label: "PM10",
      data: history.map(item => item.pollutants?.pm10 ?? 0),
      backgroundColor: "#f97316"
    },
    {
      label: "NO₂",
      data: history.map(item => item.pollutants?.no2 ?? 0),
      backgroundColor: "#3b82f6"
    },
    {
      label: "O₃",
      data: history.map(item => item.pollutants?.o3 ?? 0),
      backgroundColor: "#22c55e"
    },
    {
      label: "SO₂",
      data: history.map(item => item.pollutants?.so2 ?? 0),
      backgroundColor: "#a855f7"
    },
    {
      label: "CO",
      data: history.map(item => item.pollutants?.co ?? 0),
        ...
        callbacks: {
          label: (ctx) => {
            ...
            return [
              ...
              `Overall Condition: ${info.category}`,
              info.impact
            ];
          }
        }
      ...
      tooltip: {
        callbacks: {
          label: (ctx) => {
            ...
            const info = getAQIDetails(value);
            return [
              `Hourly AQI Reading: ${value}`,
              `Current Level: ${info.category}`,
              info.impact
            ];
          }
        }
      ...
      <DashboardLayout>
        ...
        <div className={styles.page}>
          ...
          <section className={`${styles.hero} ${styles.fadeIn}`}>
            ...
            <p>
              Analyze AQI trends, pollutant distribution, and real-time air quality patterns.
            </p>
          </section>
          ...
          {!loading && mode === "daily" && (
            <>
              <div className={styles.largeCard}>
                <Line data={dailyAQIChart} options={aqiOptions} />
              </div>
              <div className={styles.largeCard}>
                <Bar data={pollutantChart} options={pollutantOptions} />
              </div>
            </>
          )}
          ...
          {!loading && mode === "detailed" && (
            <>
              <div className={styles.largeCard}>
                <Line data={avg5Chart} options={avg5Options} />
              </div>
              <div className={styles.largeCard}>
                <Line data={last24Chart} options={last24Options} />
              </div>
            </>
          )}
